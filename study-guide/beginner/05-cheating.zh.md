...
在 CHSH 达到 Tsirelson 上限时，Bob 的两根核验轴彼此夹角为 $90^\circ$。Alice 选正中间的轴，因此离每根核验轴都是 $45^\circ$，对应的蒙对率是
...
论文在 NPA 层级 2 上计算了一个**包含所有量子策略的放松集合**的 SDP 上界，并给出上述显式量子策略作为下界；两者在约 $10^{-8}$ 的数值精度内吻合。因此这不是逐个穷举所有盒子，而是“外部上界与内部可达策略相遇”。在该数值认证精度下，85% 是最大 CHSH、渐近情形的最优控制率。

轮数有限、盒子可以记仇时，除了显式 $1/N$ 的“第 $N+1$ 次使用”风险，还有经验 CHSH 与下一轮条件期望之间的集中误差；它依赖 $N$、阈值和优化参数，不能统称为 $1/N$。合适取极限时这些修正消失，才回到约 85%。
...
| Alice | 封上之后改口 | $\cos^2(\pi/8)\simeq 0.85$（最大 CHSH、渐近） | 盒子必须继续表现得会打 CHSH |

两边都不是 50%。量子比特承诺本来就不能两边同时完美。在最大 CHSH、无限测试的极限，本文得到一头 85%、一头 75% 的不平衡点，换来的是：**只用一对盒子、只打 CHSH、仪器内部可以完全不透明、还可以有记忆。**
