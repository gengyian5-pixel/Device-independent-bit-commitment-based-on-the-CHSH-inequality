...
在标准的非相对论、无可信第三方、无预共享设置、要求信息论安全的两方模型中，经典协议不能同时做到隐藏性和绑定性；标准量子模型中也不存在完美比特承诺。不过，允许作弊概率大于 $1/2$ 的不完美量子协议仍然存在。

这篇论文给出这样一种协议，而且不依赖设备内部维数、状态和测量实现。但“设备无关”**不是没有假设**：安全性仍建立在量子理论、可信私人随机数、诚实实验室不泄密、需要时两盒不能通信，以及每次输入都有输出之上。CHSH 统计是在这些前提下约束未知设备的可观测证据。
...
你**不许靠打开盒子确认内部实现**。厂商可能在撒谎，Alice 或 Bob 也可能事先把盒子做手脚。这就是“设备无关”：安全不能靠“说明书上写着这是光子偏振计”，而要靠协议假设和实际输入输出统计共同证明。
...
假设 Bob 固定按 $y=0$。无论 Alice 按 $x=0$ 还是 $x=1$，联合概率都可能改变，但 Bob 的边际保持不变：

| Alice 输入 $x$ | $P(00\mid x,0)$ | $P(01\mid x,0)$ | $P(10\mid x,0)$ | $P(11\mid x,0)$ | Bob 见 0 的概率 | Bob 见 1 的概率 |
|---:|---:|---:|---:|---:|---:|---:|
| 0 | 0.50 | 0 | 0 | 0.50 | $0.50+0=0.50$ | $0+0.50=0.50$ |
| 1 | 0.25 | 0.25 | 0.25 | 0.25 | $0.25+0.25=0.50$ | $0.25+0.25=0.50$ |

第一行两边总相同，第二行完全随机，**相关结构不同**；但 Bob 单看自己，两个输入下都只见到各 50% 的 0/1，因此不知道 Alice 选了什么。注意表头 $P(r^0r^1\mid x,0)$ 中第二位输出属于 Bob。
