...
- 检查令牌（token）的一致性：仅当 $q=r^c$ 或 $q=r^c\oplus b$ 时接受。（为什么是“或”？因为 $q=r\oplus ab$，所以当 $b=0$ 时必须有 $q=r$；当 $b=1$ 时，根据 $a$ 的取值，$q=r$ 和 $q=r\oplus 1$ 都可能发生。）
- 在时刻 $t_{n+1}>t^d$，Bob 输入 $s^{\bar c}_{n+1}=s^c_{n+1}-2=b$；除非 $r^{\bar c}_{n+1}=r^c_{n+1}$，否则中止。

**习题 6.1。** 证明：若 Alice 诚实，则对于 $b$ 的两个取值，$q=r^c\oplus ab$ 总能通过令牌检查。
