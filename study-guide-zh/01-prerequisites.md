...
> **还没学过量子力学？** 先读白话路径，不要从下面的公式起头：
> [beginner/README.md](../study-guide/beginner/README.md)（从「比特、黑盒子」讲到整篇协议）。
> 读完再回来做本节习题。英文短导读：[00-no-quantum.md](../study-guide/00-no-quantum.md)。

[English](../study-guide/01-prerequisites.md)
...
|---|---|
| 盒子、输入 $s$、输出 $r$ | 按钮和灯 |
| 无信令 | 只看自己的灯，猜不出对方按了哪个键 |
| $I$ | CHSH 答题游戏打得有多好 |
| $|I|\le 2$ | 事先约定的上限（约 75% 胜率） |
| $2\sqrt{2}$ | 量子盒子能打到的最好（约 85%，不是满分） |
| POVM $\Pi_{r\|s}$ | “按键 $s$ 时灯亮 $r$”的内部规则（你仍然不打开盒子） |
| 鞅 / Azuma | 盒子就算记仇，平均分也不能无缘无故飙走 |
